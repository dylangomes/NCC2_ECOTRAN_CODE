function product_VAR = f_VarianceMultiplication_12132018(mean_A, VAR_A, mean_B, VAR_B)
...
product_VAR                     = (mean_A .* mean_B).^2 .* ((VAR_A ./ (mean_A.^2)) + (VAR_B ./ (mean_B.^2)));
product_VAR(isnan(product_VAR))	= 0;                % fix div/0 errors
product_VAR                  	= abs(product_VAR);	% make absolute values, variance terms are all positive
