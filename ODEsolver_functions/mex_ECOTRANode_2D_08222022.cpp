...
public:
    ...
    static std::vector<unsigned int> cs_looky_driver; // row address(es) of driver group(s) (e.g., NO3); (vector: num_drivers X 1)
    static std::vector<double> cs_SinkLink_surface; // QQQ for 2D only; definition of linked SOURCE (surface) & DESTINY (subsurface) boxes for 2D physics ONLY; (unitless, only 0, 1, or -1); (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
...
//        std::cout << "Running PhysicalFlux.m_PhysicalFlux_intraODE" << std::endl;
        ...
        for (int row_index=0; row_index<(cs_num_drivers); ++row_index) {
            ...
            index_looky = cs_looky_driver[index_a];
...
private:
    ...
    std::vector<double> bnthNH4_transfer; // QQQ for 2D ONLY; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE);
    
public:
    unsigned int c_num_grps, c_num_boxes, c_num_drivers;
...
//         std::copy(std::begin(c_production_initial), std::end(c_production_initial), c_ProductionRates_t.begin()); // QQQ initialize c_ProductionRates_t
        ...
        // QQQ step 1c: pre-initialize bnthNH4_transfer for 2D shelf physics ONLY ------------
        array_dims = (c_num_boxes * 1 * c_num_boxes); // QQQ for 2D ONLY; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
        bnthNH4_transfer.resize(array_dims); // QQQ for 2D ONLY; declare 1 large block of memory; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
        // -------------------------------------------------------------------------
...
//        std::cout << "Class ECOTRANode: t = " << t << std::endl;
        ...
        // STEP 5: external driver biomass @ t -------------------------------------
...
//        std::cout << "Running ECOTRANode.m_calc_PhysicalFlux_t" << std::endl;
    ...
    void m_calc_CONSUMPTION_t(const state_type &c_ProductionRates_t) {
        ...
        // QQQ step 4a: for 2D shelf physics ONLY: move benthic NH4 from surface to sub-surface boxes
        for (int clm_index=0; clm_index<(c_num_bnthNH4); ++clm_index) {
            
            index_looky_bnthNH4     = c_looky_bnthNH4[clm_index]; // address of current bnthNH4 group (scalar); c_looky_bnthNH4 (vector: 1 X c_num_bnthNH4)
           
            for (int layer_index=0; layer_index<(c_num_boxes); ++layer_index) {     // SOURCE box address (layer)
                for (int row_index=0; row_index<(c_num_boxes); ++row_index) {       // DESTINY box address (row)
                    
                    index_a = (layer_index * c_num_boxes * 1) + (row_index * 1) + 0; // index for cs_SinkLink_surface & bnthNH4_transfer; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE); changes with row_index & layer_index
                    index_b = (layer_index * 1 * c_num_grps) + (0 * c_num_grps) + index_looky_bnthNH4; // index for consumption_IN = current bnthNH4 pool within SOURCE box, for moving bnthNH4 pools from surface boxes to sub-surface boxes; (3D matrix: 1 X num_grps X num_boxes); only changes with layer_index
                    
                    bnthNH4_transfer[index_a]   = consumption_IN[index_b]; // (mmole N/m3/d); input into current bnthNH4 pool within SOURCE box
                    bnthNH4_transfer[index_a]   = bnthNH4_transfer[index_a] * PhysicalFlux::cs_BoxVolume_t[layer_index]; // multiply by SOURCE box volume; absolute amount of input into current bnthNH4 pool within SOURCE box; (mmole N/d);
                    bnthNH4_transfer[index_a]   = bnthNH4_transfer[index_a] * PhysicalFlux::cs_SinkLink_surface[index_a]; // absolute gain or loss of bnthNH4 to DESTINY box; (mmole N/d);
                    bnthNH4_transfer[index_a]   = bnthNH4_transfer[index_a] / PhysicalFlux::cs_BoxVolume_t[row_index]; // divide by DESTINY box volume; (mmole N/m3/d); NH4 transfer from surface SOURCE to sub-surface DESTINY box

                    //bnthNH4_transfer[index_a] = (consumption_IN[index_b] * PhysicalFlux::cs_BoxVolume_t[layer_index] * PhysicalFlux::cs_SinkLink_surface[index_a]) / PhysicalFlux::cs_BoxVolume_t[row_index];

                    
                } // end for row_index (DESTINY box)
            } // end for layer_index (SOURCE box)
            ...
            for (int layer_index=0; layer_index<(c_num_boxes); ++layer_index) {     // SOURCE box address (layer)
                for (int row_index=0; row_index<(c_num_boxes); ++row_index) {       // DESTINY box address (row)
                    ...
                    index_a = (layer_index * c_num_boxes * 1) + (row_index * 1) + 0; // index for cs_SinkLink_surface & bnthNH4_transfer; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE); changes with row_index & layer_index
                    index_c = (row_index * 1 * c_num_grps) + (0 * c_num_grps) + index_looky_bnthNH4; // index for consumption_IN = current bnthNH4 pool within DESTINY box, for moving bnthNH4 pools from surface boxes to sub-surface boxes; (3D matrix: 1 X num_grps X num_boxes); only changes with row_index
            
                    consumption_IN[index_c] = consumption_IN[index_c] + bnthNH4_transfer[index_a]; // this both adds bnthNH4 to DESTINY box and removes bnthNH4 from SOURCE box

                } // end for row_index (DESTINY box)
            } // end for layer_index (SOURCE box)
            
        } // end for clm_index (different bnthNH4 pools, if there were more than 1)
        // -------------------------------------------------------------------------

        
        // step 4b: pool pelagic NH4 & benthic NH4 in all boxes
        ...
        for (int layer_index=0; layer_index<(c_num_boxes); ++layer_index) {
            for (int clm_index=0; clm_index<(c_num_grps); ++clm_index) {
                ...
                dxdt[index_a] = c_pb_t[index_a] * (
                                                   (c_TransferEfficiency[index_a] * (consumption_IN[index_a] + c_ptr_PhysicalFlux[0]->c_flux_domain_import_driver_t[index_a] + c_ptr_PhysicalFlux[1]->c_flux_domain_import_driver_t[index_a]))
...
std::vector<double> PhysicalFlux::cs_SinkLink_surface; // QQQ for 2D only; definition of linked SOURCE (surface) & DESTINY (subsurface) boxes for 2D physics ONLY; (unitless, only 0, 1, or -1); (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
...
public:
    ...
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        // QQQ definition of linked SOURCE (surface) & DESTINY (subsurface) boxes for 2D physics only
        matlab::data::TypedArray<double> mat_SinkLink_surface                 = std::move(PhysicsStruct_mat[0][PhysicsFieldNames[41]]); // QQQ for 2D only; definition of linked SOURCE (surface) & DESTINY (subsurface) boxes for 2D physics only; (unitless, only 0, 1, or -1); (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
        // -----------------------------------
        ...
        // QQQ for 2D shelf physics ONLY, SinkLink_surface ---------------
        std::vector<double> SinkLink_surface(num_boxes * 1 * num_boxes); // QQQ for 2D only; declare 1 large block of memory; (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE)
        std::move(std::begin(mat_SinkLink_surface), std::end(mat_SinkLink_surface), SinkLink_surface.begin()); // QQQ for 2D only;
        // *************************************************************************
        ...
        // step 3h: build pointer vectors to interpolation functions ---------------
        // ConsumptionBudget
        ...
        ptr_InterpolatePhysicalFlux.resize(7); // initialize vector size to all 0; (vector: 7 X 1)
        ...
        PhysicalFlux::cs_SinkLink_surface.resize(num_boxes * 1 * num_boxes); // QQQ for 2D only; initialize vector size to all 0; (3D matrix: num_boxes X 1 X num_boxes)
        ...
        std::copy(std::begin(SinkLink_surface), std::end(SinkLink_surface), PhysicalFlux::cs_SinkLink_surface.begin()); // QQQ for 2D only; definition of linked SOURCE (surface) & DESTINY (subsurface) boxes for 2D physics; (unitless, only 0, 1, or -1); (3D matrix: num_boxes DESTINY X 1 X num_boxes SOURCE); NOTE use of copy not move
        // -------------------------------------------------------------------------
        ...
        ECOTRANode E2Etest(num_grps, num_nutrients, num_bnthNH4, num_eggs, num_ANYdetritus, num_livingANDfleets, num_boxes, num_drivers, num_fluxes, looky_nutrients, looky_eggs, looky_livingANDfleets, looky_ANYdetritus, looky_plgcNH4, looky_bnthNH4, ptr_InterpolatePhysiology, ptr_InterpolatePhysicalFlux, ptr_PhysicalFlux, EnergyBudget, fate_feces, fate_metabolism, fate_eggs, fate_predation, fate_senescence, TransferEfficiency, FunctionalResponseParams, production_initial);
        // -------------------------------------------------------------------------
