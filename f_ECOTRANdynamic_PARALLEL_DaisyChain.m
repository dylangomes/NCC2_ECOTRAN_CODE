function f_ECOTRANdynamic_NCC2_PARALLEL_05152023(setWD, Model_name, START,END, Region,upwelling_driver, CUTI_LAT, ...
                                                 ...
                                                 switch_INITIALproduction, switch_MonteCarlo, num_MC, FileOffset, current_MC_Offset, ShowOutput,ClimScenario)
% a function version of ECOTRANdynamic_NCC2_12082022 for batch runs
% run a dynamic model over time
% PARALLEL COMPUTING VERSION
%
% calls:
%       f_readEwEcsv_10pp_07072021                      read in ECOPATH model parameters from VisualBasic .csv file and store as variable 'dat'; (use for VisualBasic food web files with 10 primary producers)
%       (f_readKpath_05052022)                          Alternative: read the VisualBasic (ECOPATH) mass-balanced food web parameters directly from the VisualBasic Excel file as variable .dat
...
%       f_ScenarioGenerator_10212020                    use for food web restructuring scenarios
%
%       physical model:
%           f_OrdinalDate                               calculate ordinal dates; day of year with January 1 of any year = 1
%           f_ECOTRANphysics_NCC2_upwelling_05142023	NCC 5-box cross-shelf ecosystem: define the physical geometry of the model and prepare physical advection, mixing, and sinking rate time-series
%               calcur_res.mat                              dataset with monthly mean salinity, temperature, and NO3+NO2 data. (See Ruzicka et al. Journal of Marine Systems 187 (2018) 235-249)
%               f_prep_ERD_CUTI_SELECTYEAR_10132022         process ERD CUTI [https://mjacox.com/upwelling-indices/] timeseries for upwelling flux cropped to datestart:dateend time period (m2s-1). As an input, this function can trim the above timeseries out of “ALL” years (1988-2021), any individual year (e.g., “2009”), or will generate an average timeseries (one year in length) of all years (1988-2021) with an input of “AVG”. If “AVG” is selected, this one average year will be repeated for the duration of datestart:dateend; the same is true for any individual year, such as “2009”.
...
%           f_ECOTRANmigration_NCC_2D_09042022          area of overlap of neighboring model sub-domains
...
%
% returns:
%       store_T                     time-series of day numbers
...
%
% NOTE: to compile mex ODE solver (MUST have boost installed):
...
if test_budget > 0.99 % if this is true, do sequestration first, then adjust metabolism to leave a minimum 1% available to predation
...
% dat                  	= f_readKpath_05052022(readFile);       % Alternative: read the VisualBasic (ECOPATH) mass-balanced food web model directly from the VisualBasic Excel file as variable .dat
% -------------------------------------------------------------------------
...
SaveFile_label          = strcat(regexprep(Model_name, ".csv", ""), "_", ClimScenario);
% *************************************************************************
...
switch switch_MonteCarlo
    ...
    case 'MonteCarlo_build' % generate (and save) a stack of Monte Carlo models
        ...
        ECOTRAN.num_MC                              = num_MC; % SSS set number of Monte Carlo models to generate
        ...
        ECOTRAN_PEDIGREE                            = f_E2Epedigree_08042020(ECOTRAN, PEDIGREE);
...
switch switch_FoodWebScenario
    ...
    case 'FoodWebScenario_ON' % force changes to food web before dynamic run
        ...
        if max(TestInput_scenario > 0) 
            EnergyBudget_MC_temp(:, looky_nutrients, :)	= 0; % deactivate nutrient recycling; set nutrient columns in EnergyBudget to 0 so that recycled nutrients don't flow into any other group
        end
       ...
       % step 6e: force change(s) to the food web ------------------------
        ...
        modify_consumer_prior = []; % initialize BEFORE the loop;
        fate_predation_MC = ECOTRAN_MC.fate_predation; % initialize BEFORE the loop;
        
   for i = 1:size(CHANGEGROUP,2)
        ScenarioConditions.modify_consumer         	= [CHANGEGROUP(i)];                % row number(s) of consumer group(s) to force-modify
        ScenarioConditions.ScaleFactor            	= SCALE(i);                     	% value > 1 means increase flow to modify_consumer (and reduced flow to offset_consumer); < 1 means decrease flow to modify_consumer; 
        ...
        ScenarioConditions.offset_consumer([looky_nutrients; looky_ANYdetritus; modify_consumer_prior]) = [];	% (assume no change to detritus or to nutrients; --YOU ARE FREE TO CHANGE THIS ASSUMPTION--!!!)
...
%         ScenarioConditions.fate_predation_base   	= ECOTRAN_MC.fate_predation; 	% fate of production among all predators in base model; (3D matrix: num_livingANDfleets X num_grps X num_MC); NOTE: using fate_predation with nutrient recycling deactivated
        ScenarioConditions.fate_predation_base   	= fate_predation_MC; 	% YOU NEED TO CARRY FORWARD CUMULATING CHANGES OF fate_predation each loop; fate of production among all predators in base model; (3D matrix: num_livingANDfleets X num_grps X num_MC); NOTE: using fate_predation with nutrient recycling deactivated
...
% THESE ARE THE CUMULATING CHANGES YOU NEED TO CARRY THROUGH EACH LOOP
fate_predation_MC               = StaticScenario_results_1.fate_predation_scenario; % you need to build upon the previous scenario modifications with each iteration of the loop
modify_consumer_prior          	= [modify_consumer_prior; CHANGEGROUP(i)]; % build-up a list of modify_consumers at each loop interation to remove from offset_consumer list

end
        % -----------------------------------------------------------------
% size(fate_predation_MC)
% modify_consumer_prior

        % step 6f: replace EnergyBudget_MC & ConsumptionBudget_MC with the modified food web
        %% should I delete this next line?
        EnergyBudget_MC                             = StaticScenario_results_1.EnergyBudget_scenario; % you can delete this line because its really the fate_predation that gets modified and is used to 
...
if ShowOutput
    disp('NOTICE: SINKING IS ACTIVE ONLY FOR PLGC_DETRITUS')
end
...
% % SSS set DVMspeeds
% %       NOTE: DVM speed calculations done outside of ECOTRAN (e.g. file "DVMcalibration_GoMexOcn_08232021.xlsx")
...
% % MESOpelagic<<-->>EPIpelagic migration
...
%% *************************************************************************
% STEP 7: physiological modification terms---------------------------------
...
%% *************************************************************************
% STEP 10: define Michaelis-Menten functional predator-prey relations------
%         NOTE: for primary producers
% step 10a: initialize Michaelis-Menten parameters ------------------------
...
% *************************************************************************
% STEP 11: apply batch tuning adjustments to the ConsumptionBudget (only to "type" model at top of MonteCarlo stack)
...
% SSS set nitrification of plgcNH4 to 20% following Yool et al 2007
ConsumptionBudget_MC(2, looky_plgcNH4, :)   = 0.2; % nitrification
ConsumptionBudget_MC(4, looky_plgcNH4, :)   = 0.8; % 1 - nitrification
if ShowOutput
    disp('NOTICE: pelagic NH4 conversion to NO3 set to 20% & predation set to 80% in ConsumptionBudget')
end
% *************************************************************************
...
%% *************************************************************************
% STEP 12: adjustments to ConsumptionBudget--------------------------------
...
% step 12a: Adjust terminal benthic detritus in ConsumptionBudget ---------
...
% % step 12b: apply and/or test alternate ConsumptionBudget terms------------
...
% % step 12c: set senescence term(s) for primary producers to NPZD model values
...
% step 13b: pack BoxType definitions into ODEinput ------------------------
...
for MonteCarlo_loop = 1:num_MC
    ...
    % current_FunctionalResponseParams      = FunctionalResponseParams(:, :, MonteCarlo_loop);  % NOTE DE-ACTIVATE IF NOT USING MONTE CARLO FOR FUNCTIONAL RESPONSE; (2D matrix: CONSUMERS (num_grps) X prey group (num_grps)) replicated across clms (= producers)
    current_FunctionalResponseParams      = FunctionalResponseParams(:, :, 1);                  % NOTE: ACTIVATE IF NOT USING MONTE CARLO FOR FUNCTIONAL RESPONSE; (2D matrix: CONSUMERS (num_grps) X prey group (num_grps)) replicated across clms (= producers)
    ...
    % step 14b: build-up spatial sub-models -------------------------------
    ...
    switch switch_SubModel
        ...
        case 'identical_SubModel'  % OPTION 1: use the same food web across the shelf ----------------
            ...
            if ShowOutput
                disp('NOTICE: only have bnth detritus TE < 1 in sub-surface boxes?')
            end
        ...
        % FFF future code could easily allow for Monte Carlo models for each sub-region
            ...
            elseif strcmp(Region, 'NCA')
                ...
                if ShowOutput
                    error('ERROR: Choose a valid subregion: (WA,CR,NOR,SOR,NCA)')
                end
            end   
    ...
    % step 14c: calculate fate_predation ----------------------------------
    ...
    % step 14d: build time-series of varying physiologies -----------------
    ...
    % % step 14e: OPTIONAL: make scenario changes to ConsumptionBudget ----
    ...
    % step 14f: pack variables needed for ODE -----------------------------
